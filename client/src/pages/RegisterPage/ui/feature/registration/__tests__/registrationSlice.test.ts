...
describe('registrationSlice', () => {
  describe('registrationRequest', () => {
    it('registrationRequest устанавливает isLoading в true и не имеет ошибок', () => {
      ...
      const nextState = registrationReducer(initialState, registrationRequest(payload));
  ...
  describe('registrationSuccess', () => {
    it('устанавливает isLoading в false и не имеет ошибок', () => {
      const nextState = registrationReducer(initialState, registrationSuccess());
  ...
  describe('registrationFailure', () => {
    it('сбрасывает isLoading и устанавливает ошибки', () => {
      const errors = {
        ...
        password: 'Пароль должен содержать как минимум одну букву верхнего регистра'
      };
      const nextState = registrationReducer(initialState, registrationFailure(errors));
    ...
    it('обрабатывает пустые ошибки', () => {
      const nextState = registrationReducer(initialState, registrationFailure({}));
  ...
  describe('clearFieldError', () => {
    it('удаляет ошибку для конкретного поля', () => {
      ...
      const nextState = registrationReducer(stateWithErrors, clearFieldError('email'));
    ...
    it('не изменяет состояние если поле не существует', () => {
      ...
      const nextState = registrationReducer(stateWithErrors, clearFieldError('nonexistent'));
  ...
  describe('clearAllErrors', () => {
    it('очищает все ошибки', () => {
      ...
      const nextState = registrationReducer(stateWithErrors, clearAllErrors());
    ...
    it('не изменяет состояние если ошибок нет', () => {
      const nextState = registrationReducer(initialState, clearAllErrors());
